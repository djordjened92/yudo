...
def rbox_iou_d2(dt, gt):
    ...
    dt[..., -1] = torch.rad2deg(torch.where(dt[..., -1] < torch.pi,
                                      -dt[..., -1],
                                      2*torch.pi - dt[..., -1]))
    gt[..., -1] = torch.rad2deg(torch.where(gt[..., -1] < torch.pi,
                                      -gt[..., -1],
                                      2*torch.pi - gt[..., -1]))
    # Rotated boxes iou
    gt_rb = RotatedBoxes(gt)
    dt_rb = RotatedBoxes(dt)
...
def non_max_suppression(prediction, box_w, box_h, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False):
    """Runs Non-Maximum Suppression (NMS) on inference results

    Returns:
         list of detections, on (n,7) tensor per image [xywha, conf, cls]
    """
    xc = prediction[..., 3] > conf_thres  # candidates
...
def nms_rotated(boxes, scores, iou_thresh):
    ...
    while order.shape[0] > 0:
        ...
        curr_box = boxes[index]
        ious = rbox_iou_d2(curr_box[None], boxes[order])

        left = ious < iou_thresh
        order = order[left[0]]
