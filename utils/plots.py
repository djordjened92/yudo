...
def plot_one_box(x, angle, img, color, label=None, line_thickness=3):
...
def plot_images(images, targets, color, names=None, out_dir=None, paths=None, max_size=640):
    ...
    tl = 2  # line thickness
    tf = max(tl - 1, 1)  # font thickness
    _, h, w, _ = images.shape  # batch size, _, height, width
    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            if image_targets.shape[1] == 5: # training time
                boxes = np.concatenate((image_targets[:, 2:4]-[20/w, 35/h], image_targets[:, 2:4]+[20/w, 35/h]), axis=-1).T
        ...
        if out_dir:
            Image.fromarray(img).save(os.path.join(out_dir, f'{i}.jpg'))  # PIL save
        images[i] = img
    return images
